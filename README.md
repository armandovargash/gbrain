...
- **Memorable (procedural memory)**: optional, off by default. Turns a finished session's tool-call trace into a replayable *procedure* stored in **your own** brain database. See the section below.
...
- **On/off is explicit and sticky.** `memorable enable | disable | forget` sets a fail-closed consent flag stored on the source row itself; anything other than an explicit opt-in refuses writes. The gbrain-side relay is additionally gated behind `integrations.memorable.enabled` in your gbrain config (default: absent = off), which only the literal boolean `true` opens, plus a `GBRAIN_MEMORABLE` kill switch that accepts `0`, `false`, `off` or `no`. `memorable enable` writes that config key for you, and `memorable disable` clears it.
- **What gbrain itself does, in full.** When (and only when) the key is `true`, session-end appends one line to `~/.gbrain/integrations/hooks/session-receipts.jsonl` (0600, same directory and same never-throw contract as `heartbeat.jsonl`) and detach-spawns the local `memorable` binary. gbrain opens no socket and sends nothing off-machine; any network call is the CLI's, on your machine, after its own consent check.

Install and connect: the CLI is the npm package `memorable-cli` ([memorable.sh/docs](https://www.memorable.sh/docs/api)) and it resolves your existing gbrain connection automatically — same config file, same env vars; there is nothing new to authenticate against your database. Setup is `npx memorable-cli login` once (a browser sign-in), then `memorable init gbrain && memorable enable`. Consent starts OFF.
