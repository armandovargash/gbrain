...
- **Post-wave run (Phase 7):** recorded here when it lands — REQUIRED before
  /ship; per-fix attribution reads the MECHANICAL telemetry (per-verdict
  `rescued` flags, `quote_verify` counters), judge-scored deltas stay
  directional at n=1.
